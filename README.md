...
当前版本：**M2.5-4.4（v17 / API19fix）**，包名 `com.tencent.qqlite`，构建日期 2026-08-10。
...
├── README.md                  ← 本文件（自述文档）
├── 安装包/
│   └── QQMax-Ti_M2.5-4.4-API19fix-v17.apk   ← 当前可安装包（5-dex，腾讯原签名保留）
├── 文档/
...
1. 把 `安装包/QQMax-Ti_M2.5-4.4-API19fix-v17.apk` 传到手表（`adb install -r` 或直接拷贝到手表存储点击安装）；
