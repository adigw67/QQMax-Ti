...
**圆屏模式（全面适配）**：将原「圆表适配（实验性）」扩展为完整的圆屏模式——统一圆屏几何/安全区工具层 `lib/RoundWatch`，修复 API 19 发行版的圆屏检测（方形/近方形显示矩阵启发式），并把安全区、更大点击目标、背景图内切圆裁剪接入设置页、列表/网格、对话框、标题栏、输入栏、导航等全部界面。真圆屏/方形屏设备自动启用，也可在设置 → 外观主题 → 圆屏模式 手动开关。详见 `docs/ROUND-SCREEN.md`。
