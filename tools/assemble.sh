...
CP="$TOOLS_DIR:$DEXLIB2_JAR:$MULTIDEXLIB2_JAR:$GUAVA_JAR"
...
for f in "$MIXIN_DIR"/classes*.dex; do
  if dexdump "$f" 2>/dev/null | grep -q 'boost_multidex/BoostMultiDex;'; then BOOST_DEX="$f"; fi
  if dexdump "$f" 2>/dev/null | grep -q 'watch/app/WatchApplication;'; then WATCH_DEX="$f"; fi
done
